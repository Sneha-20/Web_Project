...
let create = async (req: any, res: any, next: any) => {
  ...
  let result: any = {};
  try {
    let recordCreate = await prisma.quoteAprovalStatusL1.create({
      data: {
        name,
        remark,
        active      
...
let getAll = async (req: any, res: any, next :any) => {
  let result: any = {};
  try{
  const data = await prisma.quoteAprovalStatusL1.findMany({
  });
...
let getOne = async (req: any, res: any, next :any) => {
  ...
  let result: any = {};
    try{
    const data = await prisma.quoteAprovalStatusL1.findUnique({
