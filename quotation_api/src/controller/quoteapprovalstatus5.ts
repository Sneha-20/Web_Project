...
let create = async (req: any, res: any, next: any) => {
  ...
  let result: any = {};
  try {
    let recordCreate = await prisma.quoteAprovalStatusL5.create({
      data: {
        name,
        remark,
        active      
...
let getAll = async (req: any, res: any, next :any) => {
  let result: any = {};
  try{
  const data = await prisma.quoteAprovalStatusL5.findMany({
  });
  if(data.length == 0){
...
let getOne = async (req: any, res: any, next :any) => {
  ...
  let result: any = {};
    try{
    const data = await prisma.quoteAprovalStatusL5.findUnique({
